...
export function IconNotification({ icon, showAmount, size, color, style }: IconNotificationProps) {
    const navigation = useNavigation();
    const { listUnread } = useNotifications()
    const { data: unreadNotifications } = listUnread
    const amount = unreadNotifications && unreadNotifications?.length > 9 ? "9+" : unreadNotifications?.length
    ...
    return (
        ...
        >
            ...
            {unreadNotifications && unreadNotifications?.length > 0 && !showAmount && <Box className="bg-red-500 rounded-full w-3 h-3 absolute top-0 right-[-2px]" />}
