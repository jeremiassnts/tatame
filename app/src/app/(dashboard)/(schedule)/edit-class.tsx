...
import { singleWeekDayClassFormSchema } from "~/src/constants/schemas/class-schema";
import { editClass } from "~/src/api/edit-class";
...
export default function EditClass() {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<z.infer<typeof singleWeekDayClassFormSchema>>({
    resolver: zodResolver(singleWeekDayClassFormSchema),
    defaultValues: {
      title: name,
  ...
  const { mutateAsync: editClassFn, isPending: isEditingClass } = useMutation({
    ...
    onError: (error) => {
      console.log(error);
      Alert.alert("Erro ao editar aula", "Tente novamente mais tarde");
    },
  });
  const [isSameAddress, setIsSameAddress] = useState(
    address === userProfile?.gym.address
  );
  ...
  async function onSubmit(data: z.infer<typeof singleWeekDayClassFormSchema>) {
    const { accessToken } = await getSession();
    editClassFn({
      data: {
        id,
        ...
        dayOfWeek: data.weekDay as DayOfWeek,
      ...
      {!isLoadingUserProfile && (
        <View className="pt-4">
          ...
          <Text className="font-sora text-neutral-400 text-[14px]">
            Altere os dados necessários e salve para atualizar a aula
          </Text>
        </View>
      )}
          ...
          <View className="w-full mt-3">
            ...
            <View className="flex flex-row gap-2 items-center justify-center">
              <DateTimePicker
                ...
                mode="time"
                className="w-[49%] mb-2"
                value={new Date(watch("startTime"))}
              />
              <DateTimePicker
                setNewDate={(date: Date | undefined) => {
                  if (date) {
            ...
            <FormSelect
              ...
              selectedValue={watch("weekDay")}
              viewClassName="mb-2"
            />
            <ErrorLabel error={errors?.weekDay?.message} />
            <FormSelect
              items={
                instructors
                  ? instructors?.map((g) => ({
